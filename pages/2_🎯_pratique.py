...
with col2:
    with st.container(border=True):
        ...
        st.markdown("<p style=\"font-size: 1.05em; color: #4b5563; line-height: 1.7; text-align: justify; margin-bottom: 20px;\">Développe la partie invisible des applications : bases de données, serveurs, APIs. Tu gères la logique métier, le stockage et le traitement sécurisé des données, et tu assures que tout fonctionne correctement côté serveur.</p>", unsafe_allow_html=True)
...
with col3:
    with st.container(border=True):
        st.markdown('<p style="font-size: 1.8em; font-weight: bold; color: #c81e70; margin-bottom: 15px; text-align: center;">🎮 Développement de Jeux</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 1.05em; color: #4b5563; line-height: 1.7; text-align: justify; margin-bottom: 20px;">Crée des jeux vidéo en programmant le gameplay, les graphismes et les interactions. Tu donnes vie à des univers virtuels, développes des mécaniques de jeu et optimises les performances pour une expérience fluide.</p>', unsafe_allow_html=True)
...
with col4:
    with st.container(border=True):
        ...
        st.markdown("<p style=\"font-size: 1.05em; color: #4b5563; line-height: 1.7; text-align: justify; margin-bottom: 20px;\">Conçoit l'interface visuelle des sites web et applications. Tu transformes les maquettes en code, crées des animations et tu t'assures que l'expérience utilisateur soit intuitive et responsive sur tous les appareils.</p>", unsafe_allow_html=True)
